	...
	public void onUpdate() {
		...
		if(!(this.equals(Minecraft.getMinecraft().renderViewEntity) && Minecraft.getMinecraft().gameSettings.thirdPersonView == 0)) {
			...
			if(i == 21) {
				...
				for (int j = 0; j < 2; ++j) {
					this.worldObj.spawnParticle("portal", this.posX + (this.rand.nextDouble() - 0.5D) * (double) this.width, this.posY - this.yOffset + this.rand.nextDouble() * (double) 2.5D, this.posZ + (this.rand.nextDouble() - 0.5D) * (double) this.width, (this.rand.nextDouble() - 0.5D) * 2.0D, -this.rand.nextDouble(), (this.rand.nextDouble() - 0.5D) * 2.0D);
				}
			}
		}
		...
		if (par1Entity.canAttackWithItem()) {
			if (!par1Entity.func_85031_j(this)) {
				...
				if (var2 > 0 || var4 > 0) {
					...
					if (var8) {
						if (var3 > 0) {
							par1Entity.addVelocity((double) (-MathHelper.sin(this.rotationYaw * (float) Math.PI / 180.0F) * (float) var3 * 0.5F), 0.1D,
